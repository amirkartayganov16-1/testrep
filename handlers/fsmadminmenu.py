...
from .keyboard import cancel_markup
...
async def load_desc(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await bot.send_message(message.chat.id, "Цена: (только числа)")
...
async def load_price(message: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(message.chat.id, "Допускаются только числа!")
...
async def delete_data(message: types.Message):
    if message.from_user.id == ADMIN:
        results = await botttt.sql_command_all(message)
...
async def complete_delete(call: types.CallbackQuery):
    await botttt.sql_command_delete(call.data.replace('delete: ', ''))
